...
locals {
  ...
  storage_to_domain_script_args   = "-DscPath ${var.dscAgentPackageLocation} -StorageAccountName ${azurerm_storage_account.storage.name} -StorageAccountRG ${azurem.resource_group.myrg_shd} -StoragePurpose fslogix -DomainName ${var.domain} -IdentityServiceProvider ${var.identityServiceProvider} -AzureCloudEnvironment ${var.azure_cloud_name} -SubscriptionId ${data.azurerm_subscription.current.subscription_id} -AdminUserName ${local.admin_user_name} -CustomOuPath ${var.ou_path} -OUName ${var.ou_path} -ShareName ${azurerm_storage_share.FSShare} -ClientId ${var.ARM_CLIENT_ID} -SecurityPrincipalName '${local.security_principal_name}' -StorageAccountFqdn ${azurerm_private_dns_a_record.dnszone_st.name}"
}