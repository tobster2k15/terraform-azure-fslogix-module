...
resource "null_resource" "install_az_cli" {
  provisioner "local-exec" {
    command = <<EOF
      ...
      ./env/usr/bin/az account set --subscription "${data.azuread_subscription.current.id}" 
    EOF
  }
  #### Delete Temp VM via Azure CLI ###
  provisioner "local-exec" {
    command = <<EOF
    ./env/usr/bin/ az vm delete --resource-group "${azurerm_resource_group.myrg_shd.name}" --name "${azurerm_windows_virtual_machine.temp_vm_for_st_join.name}" --force-deletion
    EOF
  }
  provisioner "local-exec" {
    command = <<EOF
